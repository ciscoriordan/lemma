...
"""
    ...
    def _create_copyright_html(self):
        ...
        fm = self.generator.front_matter
        year = datetime.now().year

        holder = fm.get("copyright", {}).get("holder", "Francisco Riordan")
        extra_lines = fm.get("copyright", {}).get("extra_lines", []) or []
        tools = fm.get("tools", []) or []
        data_sources = fm.get("data_sources", []) or []

        # Format the "Dictionary created" date. download_date is normally a
        # YYYYMMDD string set at generator init. Fall back to the raw value
        # if parsing fails.
        raw_download = self.generator.download_date or ""
        try:
            created_fmt = datetime.strptime(raw_download, "%Y%m%d").strftime("%B %-d, %Y")
        except (ValueError, TypeError):
            created_fmt = raw_download or "Unknown"

        # Format the Wiktionary extraction date. Kaikki usually gives
        # YYYY-MM-DD, sometimes ISO 8601 with time. Try each in turn, then
        # fall back to the raw string.
        raw_extraction = self.generator.extraction_date
        if not raw_extraction:
            extraction_fmt = "Unknown"
        else:
            extraction_fmt = raw_extraction
            for pattern in ("%Y-%m-%d", "%Y-%m-%dT%H:%M:%S", "%Y%m%d"):
                try:
                    extraction_fmt = datetime.strptime(raw_extraction, pattern).strftime("%B %-d, %Y")
                    break
                except (ValueError, TypeError):
                    continue

        # Build the body lines. Each paragraph-equivalent block is one line
        # ending in <br />, followed by a blank <br /> line to create visible
        # spacing in the Kindle popup. Blocks above H2 headings get their own
        # blank line too.
        lines = []
        ...
        for tool in tools:
            ...
            name = _escape_html(tool.get("name", ""))
            url = tool.get("url", "")
            suffix = _escape_html(tool.get("suffix", "."))
            if url:
                anchor = f"<a href=\"{_escape_html(url)}\">{name}</a>"
            else:
                anchor = name
            lines.append(f"    {role} {anchor}{suffix}<br />")
            lines.append("    <br />")
        for extra in extra_lines:
            lines.append(f"    {_escape_html(extra)}<br />")
            lines.append("    <br />")

        lines.append("    <h2>Data Sources</h2>")
        for src in data_sources:
            name = _escape_html(src.get("name", ""))
            url = src.get("url", "")
            # NOTE: desc is rendered as-is (not escaped) so defaults can embed
            # internal <a> tags. Front-matter JSON shipped from lemma_pro is
            # trusted the same way the repo's own templates are.
            desc = src.get("desc", "")
            if url:
                anchor = f"<a href=\"{_escape_html(url)}\">{name}</a>"
            else:
                anchor = name
            lines.append(f"    {anchor}: {desc}<br />")
            lines.append("    <br />")

        lines.append("    <h2>Build Info</h2>")
        lines.append(f"    Wiktionary data extracted: {_escape_html(extraction_fmt)}<br />")
        lines.append(f"    Dictionary created: {_escape_html(created_fmt)}<br />")
...
"""
    ...
    def _create_usage_html(self):
        fm = self.generator.front_matter
        dict_name = fm.get("edition_name") or self._default_edition_name()
        ...
        feature_items = "\n".join(
            f"      <li>{_escape_html(f)}</li>" for f in features
        )
        features_block = f"    <ul>\n{feature_items}\n    </ul>" if feature_items else ""
...
<html>
  ...
  <body>
    <h2>{_escape_html(dict_name)}</h2>
    <span>{_escape_html(tagline)}</span>
...
{features_block}
    ...
    <ul>
      ...
      <li>Select "{_escape_html(dict_name)}"</li>
    </ul>
  </body>
</html>
"""
    ...
    def _default_edition_name(self):
        """Fallback edition name when front_matter doesn't supply one."""
        return f"Lemma Greek{' Basic' if not self._is_full_build else ''} Dictionary"
    ...
    def _create_opf_file(self):
        ...
        display_title = self.generator.front_matter.get("edition_name") or self._default_edition_name()
